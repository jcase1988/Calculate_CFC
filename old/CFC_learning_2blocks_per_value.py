#@profile
def CFC_learning_2blocks_per_value(subj,blocks,phase_elec,amp_elec,method,surrogate_analysis):
   ...
   # MI_output_path = '/home/jcase/data/' + subj + block + '/' + method + '/' + s_methods[stim_method] + '_e' + str(phase_elec) + '_e' + str(amp_elec)
    ...
    MI_output_path = '/Users/johncase/Documents/UCSF Data/' + subj + '/' + subj + ''.join(blocks) + '/analysis/dPAC_per_value/e' + str(phase_elec) + '_e' + str(amp_elec)
    ...
    fp_good = arange(10,21)
    fp_bandwidth_good = arange(1,2)
    ...
    MI_block = zeros((len(fp),len(fp_bandwidth),3,2))
        ...
        for iBand,band in enumerate(fp_bandwidth):
            ...
            for val in range(-1,2):
                for early_late in range(2):
                    ...
                    #If Calculating Early PAC
                    if early_late == 0:
                        #retrieve power from allstimtimes
                        pow_istim = [trial[4] for trial in allstimtimes if trial[2] == val and trial[1] < halfway]
                        ...
                        pha_istim = [trial[5] for trial in allstimtimes if trial[2] == val and trial[1] < halfway]
                        pha_istim = array([item for sublist in pha_istim for item in sublist])

                    #If Calculating Late PAC
                    else:
                                                    #retrieve power from allstimtimes
                        pow_istim = [trial[4] for trial in allstimtimes if trial[2] == val and trial[1] >= halfway]
                        stim_n[val+1,early_late] = len(pow_istim)
                    ...
                    if method == 'MI':
                        ...
                        MI_block[iFreq,iBand,val+1,early_late] = abs(mean(pow_istim*(exp(1j*pha_istim) - mean(exp(1j*pha_istim)))))
...
CFC_learning_2blocks_per_value('EC82',['B48','B92'],27,27,'dPAC',1)
