#@profile
def CFC_learning_stimulus_short(subj,block,phase_elec,amp_elec,method,stim_method,surrogate_analysis):
   ...
   # MI_output_path = '/home/jcase/data/' + subj + block + '/' + method + '/' + s_methods[stim_method] + '_e' + str(phase_elec) + '_e' + str(amp_elec)
    ...
    low_fp_bandwidths = [(6,4),(8,8),(14,8)]

    MI_block = zeros((len(low_fp_bandwidths),2))
    MI_diff = zeros((len(low_fp_bandwidths)))
    if surrogate_analysis == 1:
        MI_diff_surrogate = zeros((len(low_fp_bandwidths),kruns))

    #Calculate MI for each phase-providing central-frequencies / bandwidths
    for iFreq,f_band in enumerate(low_fp_bandwidths):
        ...
        for blk in range(0,2):
            ...
            if method == 'MI':
                ...
                MI_block[iFreq,blk] = abs(mean(pow_istim*(exp(1j*pha_istim) - mean(exp(1j*pha_istim)))))


        # difference statistic = Block 2 - Block 1
        MI_diff[iFreq] = MI_block[iFreq,1]-MI_block[iFreq,0]
        ...
        if surrogate_analysis == 1:
            ...
            for iRun in range(kruns):
                ...
                MI_diff_surrogate[iFreq,iRun] = MI_block_surrogate[1]-MI_block_surrogate[0]




    save(open(MI_output_path+'_block_diff_short','wb'),MI_diff)
    save(open(MI_output_path+'_block_short','wb'),MI_block)
    if surrogate_analysis == 1:
        save(open(MI_output_path+'_block_diff_surrogate_short','wb'),MI_diff_surrogate)
